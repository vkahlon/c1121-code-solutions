...
app.post('/api/grades', (req, res, next) => {
  ...
  if (!Number.isInteger(score) || score < 0 || score > 100) {
    throw new ClientError(400, 'Score must be between 0 to 100.');
  }
  if (!name || !course) {
    throw new ClientError(400, 'name, score, course are required fields');
  }
  const sql = `
...
app.put('/api/grades/:gradeId', (req, res, next) => {
  ...
  if (!name || !course || !score) {
    throw new ClientError(400, 'name, score and course are required fields');
  }
  const sql = `
    update "grades"
       set "name"   = $1,
